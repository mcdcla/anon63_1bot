...
# ---  Словари для токенов ---
admin_tokens = {} # токены, ожидающие активации
activated_tokens = {} # токены, которые уже были использованы

# --- База данных пользователей (пример) ---
users = {}
admins = {}  # Dictionary to store admins
...
def generate_token():
    import secrets
    return secrets.token_hex(7) # Reduced token length to 14 symbols
...
def create_copy_token_keyboard(token):
    keyboard = types.InlineKeyboardMarkup()
    copy_token_button = types.InlineKeyboardButton(text="Скопировать📑", callback_data=f"copy_token_{token}")
    keyboard.add(copy_token_button)
    return keyboard
...
def send_moderation_message(user_id, message, admin_id):
    ...
    user_link = f"tg://user?id={user_id}"  # Create link to user's profile
...
def admin_token_handler(message):
    """Обработчик команды для добавления прав администратора."""
    user_id = message.from_user.id
    if is_admin(user_id) or is_manager(user_id):
        bot.reply_to(message, "Вы уже авторизованы")
        return
...
def process_admin_token(message):
    """Проверяет токен и назначает администратора."""
    user_id = message.from_user.id
    token = message.text

    if token in admin_tokens and token not in activated_tokens:
        if admin_tokens[token] == user_id:
            users[user_id]['role'] = 'admin'
            users[user_id]['admin_since'] = time.time()
            admins[user_id] = {'name': users[user_id]['name'], 'link': users[user_id]['username'], 'since': int(time.time())} # Add to admins dict
            activated_tokens[token] = user_id  # Move token to activated tokens
            del admin_tokens[token]
            bot.reply_to(message, "The admin has successfully entered")
            send_panel(user_id)  # Show admin panel
        else:
            users[user_id]['blocked_until'] = time.time() + 240 * 3600
            del admin_tokens[token]
            bot.reply_to(message, "Недействительный токен. Вы заблокированы на 240 часов.")
    else:
        users[user_id]['blocked_until'] = time.time() + 240 * 3600
        if token in admin_tokens:
            del admin_tokens[token]
        bot.reply_to(message, "Недействительный или истекший токен. Вы заблокированы на 240 часов.")
...
def leave_rank_handler(message):
    ...
    activated_tokens.pop(user_id, None)
    start(message)
...
def admin_add_handler(message):
    ...
    try:
        user_id_to_add = int(message.text.split('_')[2])
    except (IndexError, ValueError):
    ...
    users[user_id_to_add]['admin_since'] = time.time()
    admins[user_id_to_add] = {'name': users[user_id_to_add]['name'], 'link': users[user_id_to_add]['username'], 'since': int(time.time())}
    bot.reply_to(message, "Администратор добавлен✅")
...
def callback_query(call):
    ...
    if call.data == 'start_moderate':
        ...
        bot.send_message(user_id, f"токен из 14 рандомных букв латиницы/цифр: {token}", reply_markup=create_copy_token_keyboard(token))
        bot.answer_callback_query(call.id, "Токен сгенерирован и отправлен.", show_alert=True)
        bot.send_message(user_id, "панель управления", reply_markup=create_admin_keyboard())
