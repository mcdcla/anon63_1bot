...
# Попытка получить токен из переменной окружения, если она задана.
# В противном случае, используется токен, указанный непосредственно в коде.
BOT_TOKEN = os.environ.get("7413834924:AAEqqeIU8XnkYzCIW1noJrhr_fKFzbTFoZI")  # ключ должен быть осмысленным, например, "BOT_TOKEN"

# Если переменная окружения не задана (например, при локальном запуске),
# используется токен, прописанный в коде.
if BOT_TOKEN is None:
    BOT_TOKEN = "7413834924:AAEqqeIU8XnkYzCIW1noJrhr_fKFzbTFoZI" #Укажите ваш токен
